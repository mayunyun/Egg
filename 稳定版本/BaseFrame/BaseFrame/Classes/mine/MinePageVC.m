...
- (UIView *)numerButton:(NSArray*)arr tag:(NSInteger)tag{
    ...
    for (int i=0; i<arr.count; i++) {
        UIButton *but;
        if (but==nil) {
            but = [[UIButton alloc]initWithFrame:CGRectMake(((mScreenWidth-320)/4+80)*i, 0, 100, 100)];
        }
        but.tag = tag+i;
        ...
        [but setImageEdgeInsets:UIEdgeInsetsMake(-15, (100-but.imageView.frame.size.width)/2,0, 0)];//图片距离右边框距离减少图片的宽度，其它不边
