...
- (void)configureShopcartBottomViewWithTotalPrice:(float)totalPrice totalCount:(NSInteger)totalCount isAllselected:(BOOL)isAllSelected {
    ...
    if (self.settleButton.isEnabled) {
        [self.settleButton setBackgroundColor:[UIColor orangeColor]];
...
- (void)renderWithTotalPrice:(NSString *)totalPrice {
    ...
    [attributedString addAttributes:@{NSForegroundColorAttributeName:[UIColor orangeColor]} range:[self.totalPriceLable.text rangeOfString:totalPrice]];
...
- (UIButton *)settleButton {
    if (_settleButton == nil){
        ...
        _settleButton.titleLabel.font = [UIFont systemFontOfSize:17];
