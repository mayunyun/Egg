//
//  MYYMyOrderSouSuoViewController.m
//  BaseFrame
//
...
@interface MYYMyOrderSouSuoViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
{
...
@implementation MYYMyOrderSouSuoViewController

- (UITableView *)TableView{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 50*MYWIDTH, mScreenWidth, mScreenHeight-50*MYWIDTH-NavBarHeight)];
        ...
        _tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
            ...
            [self rightBarToLastViewController:nil];
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"homesearch"] style:UIBarButtonItemStylePlain target:self action:@selector(rightBarToLastViewController:)];
    [self.navigationItem.rightBarButtonItem setTintColor:UIColorFromRGB(0xffffff)];
    
    UIView *bgview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, mScreenWidth, 50*MYWIDTH)];
    bgview.backgroundColor = BackGorundColor;
    [self.view addSubview:bgview];
    
    UIView *baiview = [[UIView alloc]initWithFrame:CGRectMake(50*MYWIDTH, 10*MYWIDTH, mScreenWidth-100*MYWIDTH, 30*MYWIDTH)];
    baiview.backgroundColor = [UIColor whiteColor];
    baiview.layer.cornerRadius = 15*MYWIDTH;
    [bgview addSubview:baiview];
    
    textfield = [[UITextField alloc]initWithFrame:CGRectMake(20*MYWIDTH, 0, baiview.width-60*MYWIDTH, 30*MYWIDTH)];
    textfield.delegate = self;
    textfield.clearButtonMode = UITextFieldViewModeWhileEditing;
    textfield.backgroundColor = [UIColor whiteColor];
    textfield.textAlignment = NSTextAlignmentLeft;
    textfield.placeholder = @"请输入商品名或订单号";
    [textfield setValue:[UIFont boldSystemFontOfSize:14] forKeyPath:@"_placeholderLabel.font"];
    [baiview addSubview:textfield];
    
    UIButton *souBut = [[UIButton alloc]initWithFrame:CGRectMake(textfield.right, 0, 30*MYWIDTH, 30*MYWIDTH)];
    [souBut setImage:[UIImage imageNamed:@"quanbudingdan_sousuo1"] forState:UIControlStateNormal];
    [souBut addTarget:self action:@selector(rightBarToLastViewController:) forControlEvents:UIControlEventTouchUpInside];
    [baiview addSubview:souBut];
...
- (void)rightBarToLastViewController:(UIButton *)button
{
    ...
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"searchText\":\"%@\"}",textfield.text],@"page":[NSString stringWithFormat:@"%li",(long)_page],@"rows":@"10"};
    [HTNetWorking postWithUrl:@"myorder?action=searchMyOrderSearchNEW" refreshCache:YES params:params success:^(id response) {
