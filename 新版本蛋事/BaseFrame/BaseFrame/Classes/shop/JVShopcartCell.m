...
@property (nonatomic, strong) UILabel *productSepartion;//商品规格
...
- (void)setupViews {
    ...
    [self.shopcartBgView addSubview:self.productSepartion];
...
- (void)configureShopcartCellWithProductURL:(NSString *)productURL productName:(NSString *)productName productPrice:(float)productPrice productCount:(NSInteger)productCount productStock:(float)productStock productSelected:(BOOL)productSelected {
    ...
    self.productSepartion.text = @"规格:28妹";
...
- (UILabel *)productNameLable {
    if (_productNameLable == nil){
        ...
        _productNameLable.font = [UIFont systemFontOfSize:17];
        _productNameLable.textColor = [UIColor colorWithRed:70/255.0 green:70/255.0 blue:70/255.0 alpha:1];
    }
    return _productNameLable;
}
- (UILabel *)productSepartion {
    if (_productSepartion == nil){
        _productSepartion = [[UILabel alloc] init];
        _productSepartion.font = [UIFont systemFontOfSize:16];
        _productSepartion.textColor = UIColorFromRGB(0xB1B2B3);
    }
    return _productSepartion;
}
- (UILabel *)productPriceLable {
    if (_productPriceLable == nil){
        _productPriceLable = [[UILabel alloc] init];
        _productPriceLable.font = [UIFont systemFontOfSize:16];
...
- (void)layoutSubviews {
    ...
    [self.productNameLable mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.shopcartBgView).offset(15);
        make.right.equalTo(self.shopcartBgView).offset(-5);
        make.height.equalTo(@30);
    }];
    [self.productSepartion mas_makeConstraints:^(MASConstraintMaker *make) {
     make.left.equalTo(self.productImageView.mas_right).offset(15);
    make.top.equalTo(self.productNameLable.mas_bottom).offset(3);
        make.right.equalTo(self.shopcartBgView).offset(-5);
        make.height.equalTo(@30);
    }];
