...
-(void)requestBadgeCount{
    ...
    [HTNetWorking postWithUrl:[NSString stringWithFormat:@"shoppingcart?action=searchShoppingCart"] refreshCache:YES params:nil success:^(id response) {
        NSArray* array = [NSJSONSerialization JSONObjectWithData:response options:NSJSONReadingMutableContainers error:nil];
        if (array.count) {
            label.text = [NSString stringWithFormat:@"%lu",array.count];
...
- (void)creatUI
{
    _tbView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, mScreenWidth, mScreenHeight - NavBarHeight - 44-5) style:UITableViewStylePlain];
...
- (UISearchBar *)searchview{
    if (_search == nil) {
        ...
        homeButtonImageView.image = [UIImage imageNamed:@"shoppcart"];
        UIButton * homeButton = [[UIButton alloc] initWithFrame:CGRectMake(0,0,25,25)];
        homeButton.backgroundColor = [UIColor clearColor];
        [homeButton addTarget:self action:@selector(rightNavBarClick:)forControlEvents:UIControlEventTouchUpInside];
        label = [[UILabel alloc] initWithFrame:CGRectMake(15,0,10,10)];
        label.text = @"0";
...
//        [buyBtn setImage:[UIImage imageNamed:@"shoppcart"] forState:UIControlStateNormal];
...
//        [self.navigationItem.rightBarButtonItem setTintColor:UIColorFromRGB(0xffffff)];
        ...
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, mScreenWidth-120, 30)];
        //加上 搜索栏
        _search = [[HXSearchBar alloc] initWithFrame:CGRectMake(0, 0, mScreenWidth - 120, 30)];
        _search.backgroundColor = [UIColor clearColor];
        _search.delegate = self;
        //输入框提示
        _search.placeholder = @"搜你想要的";
        //光标颜色
        _search.cursorColor = [UIColor whiteColor];
        UITextField *searchField = [_search valueForKey:@"_searchField"];
        searchField.backgroundColor = NavBarItemColor;
        [searchField setValue:NavBarItemColor forKeyPath:@"_placeholderLabel.backgroundColor"];
        searchField.textColor = [UIColor whiteColor];
        [searchField setValue:[UIColor whiteColor] forKeyPath:@"_placeholderLabel.textColor"];
        searchField.frame = CGRectMake(0, 0, mScreenWidth - 120, 30);
        [searchField setBorderStyle:UITextBorderStyleRoundedRect];
        //自定义放大镜图片
        UIImage *image = [UIImage imageNamed: @"search_ico2"];
        UIImageView *iView = [[UIImageView alloc] initWithImage:image];
        if (IS_IOS_7) {
            iView.frame = CGRectMake(5, 10, 17, 17);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        [cell setMoreBtnBlock:^{
            AllProductPageVC * vc = [[AllProductPageVC alloc]init];
//            HomeFavorableModel * model = _favorableDataArray[0];
//            vc.typeId = [NSString stringWithFormat:@"%@",model.type];
            vc.title = @"特惠产品";
            vc.typeId = @"";
            vc.biaoshi = @"special";
            vc.sorts = @"zonghe";
    ...
    if (cycleScrollView == self.cycleScrollView2) {
        ...
        if (!IsEmptyValue(_bannerDataArray)&&_bannerDataArray.count>index) {
            ...
            if (!IsEmptyValue(model.Id)) {
                ...
                AllProductPageVC * vc = [[AllProductPageVC alloc]init];
                vc.typeId = [NSString stringWithFormat:@"%@",model.Id];
                vc.title = [NSString stringWithFormat:@"%@",model.subjectname];
                vc.sorts = @"zonghe";
                vc.biaoshi = @"lunbo";
                [self.navigationController pushViewController:vc animated:YES];
            }
...
- (void)bannerRequestData{
    [HTNetWorking postWithUrl:[NSString stringWithFormat:@"mall/showproduct?action=subjectPic"] refreshCache:YES params:nil success:^(id response) {
        ...
        UIView *headview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, mScreenWidth, 210)];
        // 网络加载 --- 创建带标题的图片轮播器
